...
import { Camera, Plus, Trash2, ShoppingCart } from "lucide-react";
...
const SalesEntryPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const entry: SalesEntry = {
      id: Date.now().toString(), customerName, weight: w,
      rate: r > 0 ? r : undefined, amount,
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><ShoppingCart className="w-5 h-5 text-primary" /> New Sales Entry</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label>Customer Name *</Label>
              <Input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Buyer name" className="h-12" />
            </div>
            <div className="space-y-2">
              <Label>Weight (kg) *</Label>
              <Input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="0" className="h-12" />
            </div>
            <div className="space-y-2">
              <Label>Rate (₹/kg) <span className="text-muted-foreground text-xs">(optional)</span></Label>
              <Input type="number" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="Optional" className="h-12" />
            </div>
            <div className="space-y-2">
              <Label>Amount</Label>
              <div className="h-12 rounded-lg bg-accent flex items-center px-4 font-mono text-sm">
                {amount !== undefined ? <strong className="text-primary">₹{amount.toLocaleString()}</strong> : <span className="text-muted-foreground">Pending</span>}
              </div>
            </div>
      ...
      <Card>
        ...
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Customer</TableHead><TableHead className="text-right">Weight</TableHead>
            ...
            <TableBody>
              {entries.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell className="text-right font-mono">{entry.rate ? `₹${entry.rate}` : "—"}</TableCell>
                  <TableCell className="text-right font-mono font-semibold">{entry.amount ? <span className="text-primary">₹{entry.amount.toLocaleString()}</span> : <span className="text-warning">Pending</span>}</TableCell>
                  <TableCell className="text-muted-foreground text-sm">{entry.createdAt}</TableCell>
                  {hasPermission("delete_entries") && (
                    <TableCell><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setEntries((p) => p.filter((x) => x.id !== entry.id))}><Trash2 className="w-4 h-4" /></Button></TableCell>
                  )}
                </TableRow>
              ))}
            </TableBody>
